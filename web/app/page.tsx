...
// does the vision model report anything of concern?
function aiHasConcerns(ai: AI | null) {
  if (!ai) return false;
  const c = (ai.concerns || "").toLowerCase().trim();
  if (!c || c === "none visible" || c === "none" || c === "n/a") return false;
  return true;
}
function aiUrgent(ai: AI | null) {
  if (!ai) return false;
  const v = ((ai.verdict || "") + " " + (ai.condition || "")).toLowerCase();
  return /high urgency|urgent|immediate|severe|critical|poor/.test(v);
}
...
function buildAnalysis(incidents: Incident[], ai: AI | null, ctx: { mode: string; frames: number; dur: string }) {
  ...
  if (noCable) {
    ...
    if (n === 0 && concerns) {
      narr.push(`Although the specialist detector found none of its two trained fault types, the vision-model review identified other visible concerns: ${ai!.concerns} These fall outside the detector's scope and should not be dismissed.`);
    } else if (n > 0 && concerns) {
      narr.push(`In addition to the detector findings, the vision-model review noted: ${ai!.concerns}`);
    } else if (n === 0 && ai && !concerns) {
      narr.push(`The vision-model review also reported no visible concerns, giving two independent sources in agreement.`);
    } else if (n === 0 && !ai) {
      narr.push(`No vision-model review was run for this scan. A clear result from the specialist detector alone does not certify the asset as fault-free.`);
    }
...
export default function Page() {
  ...
  const logIncident = useCallback((id: number, cls: number, conf: number, box: any, src: CanvasImageSource, sw: number, sh: number, persist: number) => {
    ...
    const inc: Incident = { id, cls, sev, conf, ts: Date.now(), thumb: makeThumb(src, box, sw, sh), action: actionFor(cls, sev), areaFrac, persist, region: regionOf((box.y + box.h / 2) / sh) };
  ...
  const loop = useCallback(async () => {
    ...
    if (v.readyState >= 2) {
      if (skipRef.current > 0 && !busyRef.current) {
        skipRef.current--;
  ...
  const runAI = useCallback(async () => {
    ...
    const f = incRef.current.length
      ? incRef.current.slice(0, 5).map(i => `${NAMES[i.cls]} (${(i.conf * 100).toFixed(0)}%)`).join(", ")
      : "no faults detected";
    try {
  ...
  const onVideo = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    v.onended = () => { runningRef.current = false; setRunning(false); cancelAnimationFrame(rafRef.current); setVprog(1); setTimeout(() => runAI(), 100); setTimeout(() => setShowReport(true), 1800); };
  }, [stopCam, resetScan, fitCanvas, loop, runAI]);
  ...
  }, [stopCam, resetScan, fitCanvas, infer, draw, logIncident, runAI]);
    ...
    <div className="wrap">
      ...
      <main className="main">
        ...
        <aside className="side">
          ...
          <div className="ctl">
            {mode === "live" && (running ? <button className="btn stop" onClick={stopCam}>Stop scan</button> : <button className="btn go" onClick={startCam} disabled={ready === false}>Start scan</button>)}
            {mode === "video" && (running ? <button className="btn stop" onClick={stopCam}>Stop</button> : <label className="btn go" style={{ opacity: ready ? 1 : .45 }}>Upload footage<input type="file" accept="video/*" hidden onChange={onVideo} disabled={ready === false} /></label>)}
          ...
          <button className="btn ghost" onClick={runAI} disabled={aiBusy || !ready} style={{ width: "100%" }}>
          ...
          <div className="foot">YOLO26 · mAP50 0.871 · local + vision-model review</div>
        </aside>
      </main>
